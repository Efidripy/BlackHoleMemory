...
from blackholememory.resource_limits import BHM_FEDERATED_EMBEDDING_PREPARATION_TIMEOUT_SECONDS
...
def test_app_env_float_supports_upper_bounds(monkeypatch) -> None:
    ...
    assert BHM_FEDERATED_EMBEDDING_PREPARATION_TIMEOUT_SECONDS == 3
    ...
    assert any(item.key == "retrieval.embedding_preparation_timeout" for item in RESOURCE_LIMITS)
