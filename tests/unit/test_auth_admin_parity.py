...
def test_generated_auth_admin_parity_covers_all_static_interface_rows() -> None:
    ...
    assert report["inventory_row_count"] == 450
    assert report["classified_row_count"] == 450
    assert report["surface_counts"] == {"MCP_STATIC": 192, "REST/WS": 258}
    assert report["mcp_registration_groups"] == {"admin": 73, "core": 35, "domain": 84}
