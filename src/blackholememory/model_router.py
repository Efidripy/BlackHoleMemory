...
def route_model(
    ...
    *,
    ...
    chosen = sorted(candidates, key=lambda item: (float(item["latency_ms"]), item["model_id"]))[0]
    reasons.extend(("capability_match", "measured_context_profile", "local_only_attestation"))
    return RouteDecision("routed", task, requested, str(chosen["model_id"]), int(profile["profile_tokens"]), tuple(reasons))
