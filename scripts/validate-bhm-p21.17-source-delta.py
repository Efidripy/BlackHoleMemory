...
DISPOSITIONS = {
    ...
    "LCM": ("reference-only", "event-log/replay concepts remain a benchmark hypothesis; no pinned source and no duplicate Rust/events authority may enter BHM", [".docs/ops/bhm-p20.7-wi06-memory-graph-2026-07-16.md", "tests/integration/test_wi06_memory_graph.py"]),
    "JCG": ("reference-only", "query UX and pagination are reference criteria only; repository license remains unverified and no runtime namespace is added", ["src/blackholememory/code_graph_query.py", "tests/integration/test_wi03_code_graph_query.py"]),
    "MNM": ("reference-only", "public UX claims are not an auditable source implementation and remain optional review criteria", ["src/blackholememory/session_capture.py", "tests/integration/test_wi05_session_capture.py"]),
    ...
    "CAV": ("reference-only", "commercial hosted documentation; public UX reference only, no code or service dependency", ["src/blackholememory/unified_context.py", "tests/integration/test_wi08_unified_context.py"]),
