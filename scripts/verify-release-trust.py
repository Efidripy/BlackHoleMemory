...
def sidecar_hash(path: Path) -> str:
    if not path.exists():
        return ""
    match = re.search(r"(?i)\b[a-f0-9]{64}\b", path.read_text(encoding="utf-8"))
    return match.group(0).lower() if match else ""


def verify_files(files: dict[str, bytes], expected_version: str, failures: list[str]) -> dict[str, object]:
    ...
    evidence_by_name = {
        str(item.get("name") or "").replace("_", "-").replace(".", "-").lower(): item
        for item in (build_inputs.get("packages") if isinstance(build_inputs.get("packages"), list) else [])
...
def main() -> int:
    ...
    if args.archive:
        ...
        trust_result = verify_files(files, args.expected_version, failures)
            ...
            if not all(path is not None for path in signature_paths):
                ...
                completed = subprocess.run(
                    [
                        ...
                        "--receipt", str(args.signature_receipt),
