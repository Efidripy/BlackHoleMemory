#!/usr/bin/env python
"""Bounded metadata-only BitBake parser benchmark for WI-158."""
...
from blackholememory.code_graph import _extract_bitbake_structural  # noqa: E402
...
FIXTURES = {
    ...
    "bitbake_append": ("require demo.inc\nFILESEXTRAPATHS:prepend := \"${THISDIR}/files:\"\n", "demo_1.0.bbappend"),
}


def _run_once() -> dict[str, object]:
    output: dict[str, object] = {}
    for label, (content, path) in FIXTURES.items():
        draft = _GraphDraft("root-wi158", {})
        ...
        status, error = _extract_bitbake_structural(draft, path, file_key, content, digest)
        output[label] = {
            "status": status,
            "error": error,
...
def run(iterations: int = 1000) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        "schema_version": "bhm.p28.wi158.bitbake-parser-benchmark.v1",
        "extractor_version": CODE_GRAPH_EXTRACTOR_VERSION,
        "parser_id": PARSER_REGISTRY["bitbake"]["parser_id"],
        "languages": first,
        "iterations": count,
        ...
        "execution": {"raw_source_returned": False, "network": False, "compiler_or_lsp": False, "store_writes": False, "autonomous_apply": False},
    }
